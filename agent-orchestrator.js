...
Provide a focused response:`;
  ...
  async editFile(filePath, instructions) {
    ...
    try {
      ...
      const codeAnalysis = await this.dispatchToAgent('code', 
        `Analyze this file and apply these instructions: ${instructions}`, 
        context
      );

      spinner.text = 'Generating updated file...';
      
      const updatedContent = await this.callOpenAI(
  ...
  async runTest(testFile, expectedOutput) {
    ...
    try {
      ...
      const testAgent = await this.dispatchToAgent('test', 
        `Execute this test and validate results`, 
        { testContent, expectedOutput }
      );

      spinner.succeed('Test completed');
      return {
        success: true,
        results: testAgent.response,
        passed: testAgent.response.includes('PASSED')
      };
    } catch (error) {
  ...
  async orchestrateTask(userRequest, options = {}) {
    ...
    try {
      // Master agent analyzes the request
      ...
      // Determine task type and dispatch to appropriate agents
      let results = [];
...
async function main() {
  ...
  if (args.length === 0 || args.includes('--help')) {
    ...
    console.log('  node agent-orchestrator.js "Add error handling to server.js"');
    console.log('  node agent-orchestrator.js "Create unit tests for utils.js" --file utils.js');
    console.log('  node agent-orchestrator.js "Document the API endpoints" --show-thinking');
    return;
  }
